...
class Map():
    ...
    def make_map_with_M(self, mu, sigma2, M, OD_ori=None, mu2=None, sigma=None, phi_bi=None):
        self.mu = mu
        self.sigma2 = sigma2
        self.M = M
    ...
    def extract_map(self, map_index=0, r_0=1, r_s=15, kai=0.4):
        self.M, self.idx, self.mu = extract_map(map_index)
        self.sigma, self.sigma2 = generate_sigma3(self.mu, kai)
        self.n_node = self.M.shape[0]
        self.n_link = self.M.shape[1]
        self.b, self.r_0, self.r_s = generate_b(self.M.shape[0], r_0, r_s)
        self.G = None

    # irrelevant edge cost
    def get_edge_log_cost(self, edge_idx, K):
        sigma2 = self.sigma2[edge_idx][edge_idx]
        mu = self.mu[edge_idx]
        cov2_log = np.log(sigma2 / mu ** 2 + 1)
        mu_log = np.log(mu ** 2 / (np.sqrt(sigma2 + mu ** 2)))
        cost = np.random.lognormal(mu_log, np.sqrt(cov2_log), K)
        return cost.item()

    def get_edge_normal_cost(self, edge_idx, K):
        sigma2 = self.sigma2[edge_idx][edge_idx]
        mu = self.mu[edge_idx]
        cost = np.random.normal(mu, sigma2, K)
        return cost.item()
...
def extract_map(map_id):
    table_paths = ['Maps/SiouxFalls/SiouxFalls_network.xlsx',  # 0
                   'Maps/SiouxFalls/SiouxFalls_network_copy.xlsx',  # 1
                   'Maps/Anaheim/Anaheim_network.xlsx',  # 2
                   "Maps/Winnipeg/winnipeg_network.xlsx",  # 3
                   "Maps/Barcelona/Barcelona_network.xlsx",  # 4
                   "Maps/Eastern-Massachusetts/Eastern-Massachusetts.xlsx",  # 5
                   "Maps/Berlin-Friedrichshain/Berlin-Friedrichshain.xlsx"]  # 6
